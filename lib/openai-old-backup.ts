...
async function generateFallbackContent(request: ContentGenerationRequest): Promise<GeneratedContent> {
  ...
  // Search for dynamic media based on article content
  ...
  try {
    ...
    // Media search failed, but we continue with content generation
  }

  // Ensure we always have at least some fallback images
  if (dynamicMedia.images.length === 0) {
    console.log('🔄 Adding fallback images since no media was found...');
    dynamicMedia.images = getFallbackImagesForCategory(category);
    console.log(`✅ Added ${dynamicMedia.images.length} fallback images`);
  }
...
function createContentWithMedia(title: string, topic: string, category: string, keywords: string[], media: {
  images: MediaItem[];
  videos: MediaItem[];
  tweets: string[];
}): string {
...
${getImageMd(1, `${topic} Fundamentals`)}
...
${getImageMd(2, `${topic} Implementation`)}
...
${getImageMd(3, `${topic} Applications`)}
...
${getImageMd(4, `${topic} Risk Management`)}
...
${getImageMd(5, `${topic} Future Trends`)}
...
This comprehensive guide addresses key concepts and strategies related to: ${keywords.join(', ')}, providing practical insights, actionable strategies, and proven methodologies for successful implementation and sustainable value creation in the ${category} sector.
...
// Fallback images when MediaManager fails
...
export async function generateArticleContent(request: ContentGenerationRequest): Promise<GeneratedContent> {
  ...
  // Use the improved fallback content generation for consistent, comprehensive articles
  console.log('Using enhanced fallback content generation with dynamic media');
