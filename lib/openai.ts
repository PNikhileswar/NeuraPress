...
function createContentWithMedia(
  ...
  <iframe width="560" height="315" src="https://www.youtube.com/embed/${videoId}" 
    title="${alt}" frameborder="0" allowfullscreen></iframe>
</div>\n\n`;
      }
    }
    return '';
...
*This article represents a thorough analysis of ${topic} based on current industry research, expert insights, proven methodologies, and real-world case studies. For more information, updates on emerging trends, and additional resources, visit TrendWise regularly for the latest insights and analysis.*`;
}
...
async function generateFallbackContent(request: ContentGenerationRequest): Promise<GeneratedContent> {
  ...
  try {
    ...
    console.log(`✅ Found ${dynamicMedia.images.length} images and ${dynamicMedia.videos.length} videos`);
  } catch (error) {
    console.error('❌ Error searching for media:', error);
  }

  // Generate contextual tweets based on topic and keywords
  dynamicMedia.tweets = generateContextualTweets(topic, category, keywords);
  console.log(`✅ Generated ${dynamicMedia.tweets.length} contextual tweets`);

  // Ensure we always have at least 4 fallback images
  if (dynamicMedia.images.length < 4) {
    console.log('🔄 Adding fallback images to meet minimum requirement...');
    const fallbackImages = getFallbackImagesForCategory(category);
    
    // Add fallback images to fill the gap
    const needed = 4 - dynamicMedia.images.length;
    dynamicMedia.images.push(...fallbackImages.slice(0, needed));
    
    console.log(`✅ Added ${needed} fallback images (total: ${dynamicMedia.images.length})`);
  }

  // Ensure we always have videos
  if (dynamicMedia.videos.length < 2) {
    console.log('🔄 Adding fallback videos...');
    const fallbackVideos = getFallbackVideosForCategory(category);
    const needed = 2 - dynamicMedia.videos.length;
    dynamicMedia.videos.push(...fallbackVideos.slice(0, needed));
    
    console.log(`✅ Added ${needed} fallback videos (total: ${dynamicMedia.videos.length})`);
  }

  // Always generate contextual tweets
  dynamicMedia.tweets = generateContextualTweets(topic, category, keywords);
  console.log(`✅ Generated ${dynamicMedia.tweets.length} contextual tweets`);

  // Ensure we have at least 1 video for rich content
  if (dynamicMedia.videos.length === 0) {
    dynamicMedia.videos = getFallbackVideosForCategory(category);
    console.log(`✅ Added ${dynamicMedia.videos.length} fallback videos`);
  }
...
function generateContextualTweets(topic: string, category: string, keywords: string[]): string[] {
  const tweetTemplates = [
    `🚀 Just learned about ${topic}! This is transforming the ${category} industry. #${keywords[0]?.replace(/\s+/g, '')} #Innovation`,
    `💡 Key insight: ${topic} is reshaping how we approach ${category}. What's your take? #${keywords[1]?.replace(/\s+/g, '') || 'TechTrends'}`,
    `🔥 Breaking: ${topic} shows promising results in ${category} applications. Thread below 👇 #${keywords[2]?.replace(/\s+/g, '') || 'Industry'}`,
    `📈 The impact of ${topic} on ${category} is undeniable. Here's what you need to know... #${keywords[0]?.replace(/\s+/g, '')} #FutureOfWork`,
    `🎯 Quick thread on ${topic}: Why it matters for ${category} professionals and what's next #${keywords[1]?.replace(/\s+/g, '') || 'Professional'}`
  ];

  // Add category-specific tweets
  const categorySpecificTweets: Record<string, string[]> = {
    technology: [
      `💻 ${topic} is the next big thing in tech. Early adopters are already seeing benefits! #TechInnovation #${keywords[0]?.replace(/\s+/g, '')}`,
      `🤖 AI meets ${topic}: The future of technology is here. What are your thoughts? #AI #${keywords[0]?.replace(/\s+/g, '')}`
    ],
    business: [
      `📊 ${topic} is driving business transformation. ROI projections are impressive! #BusinessGrowth #${keywords[0]?.replace(/\s+/g, '')}`,
      `💼 Entrepreneurs: ${topic} could be your competitive advantage in 2025 #Startup #${keywords[0]?.replace(/\s+/g, '')}`
    ],
    health: [
      `🏥 ${topic} breakthrough: New possibilities for better health outcomes #HealthTech #${keywords[0]?.replace(/\s+/g, '')}`,
      `💊 Medical professionals are buzzing about ${topic}. Here's why... #Medicine #Healthcare`
    ],
    finance: [
      `💰 ${topic} is reshaping financial markets. Investment opportunities ahead! #FinTech #${keywords[0]?.replace(/\s+/g, '')}`,
      `📈 Portfolio tip: Keep an eye on ${topic} developments in the finance sector #Investment #Finance`
    ]
  };

  const specificTweets = categorySpecificTweets[category] || [];
  const allTweets = [...tweetTemplates, ...specificTweets];
  
  // Return 3-5 diverse tweets
  return allTweets.slice(0, Math.min(5, allTweets.length));
}

/**
 * Get fallback videos for specific categories
 */
function getFallbackVideosForCategory(category: string): MediaItem[] {
  const videoTemplates: Record<string, Array<{title: string; description: string}>> = {
    technology: [
      { title: 'Understanding Modern Technology Trends', description: 'Comprehensive overview of current tech innovations' },
      { title: 'AI and Machine Learning Explained', description: 'Deep dive into artificial intelligence applications' }
    ],
    business: [
      { title: 'Business Strategy Fundamentals', description: 'Essential strategies for business success' },
      { title: 'Entrepreneurship in the Digital Age', description: 'Modern approaches to starting and scaling businesses' }
    ],
    health: [
      { title: 'Health and Wellness Guide', description: 'Expert tips for maintaining optimal health' },
      { title: 'Medical Innovations Explained', description: 'Latest breakthroughs in medical technology' }
    ],
    finance: [
      { title: 'Investment Strategies for Beginners', description: 'Fundamental investment principles and strategies' },
      { title: 'Financial Planning Made Simple', description: 'Practical approaches to personal finance' }
    ]
  };

  const templates = videoTemplates[category] || videoTemplates.technology;
  
  return templates.map((template, index) => ({
    id: `fallback-video-${category}-${index}`,
    url: `https://www.youtube.com/watch?v=dQw4w9WgXcQ`, // Placeholder - in production, use real video IDs
    ...
    type: 'video' as const,
    tags: [category],
    relevanceScore: 40, // Lower than dynamic search results
    metadata: {
      duration: 630, // 10:30 in seconds
      views: 1200000,
      uploadDate: new Date().toISOString(),
      channel: 'TrendWise Education'
    }
  }));
}
